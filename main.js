...
//Functions
function setNoQuestions(num) {    // show the total number of questions
    countQuestions.innerHTML= `Question ${countQ + 1} out of ${num}`;  
}
function setQuestions(obj, num) {   // create the question and answers elements
    if (countQ < num) { // if there is still questions
        // create question
...
function countdown(duration,num) {  // counting down for each Question
    if (countQ < num) {
...
function checkAnswer(rAnswer) { // Check if the answer is right and add to score
        let chosenAnswer
...
function showResult(num) {  // remove questions and answers and show the total score
        if (countQ === num) {
